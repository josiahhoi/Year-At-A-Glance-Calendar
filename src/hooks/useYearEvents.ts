import { useQueries, useQuery } from '@tanstack/react-query';
import { useMemo } from 'react';
import { listEventsRange } from '../api/events';
import { appEventFromWire, type AppEvent } from '../model/eventModel';

export function yearEventsKey(calendarId: string, year: number) {
  return ['events', calendarId, 'year', year] as const;
}

/**
 * All events of the glance calendar overlapping the given year.
 * timeMin/timeMax select by overlap, so events spilling across New Year
 * are returned for both adjacent years.
 */
async function fetchYearEvents(calendarId: string, year: number): Promise<AppEvent[]> {
  const wire = await listEventsRange(
    calendarId,
    `${year}-01-01T00:00:00Z`,
    `${year + 1}-01-01T00:00:00Z`,
  );
  return wire.map((w) => appEventFromWire(w, calendarId)).filter((e): e is AppEvent => e !== null);
}

export function useYearEvents(calendarId: string | null, year: number) {
  return useQuery<AppEvent[]>({
    queryKey: yearEventsKey(calendarId ?? 'none', year),
    enabled: !!calendarId,
    staleTime: 60 * 1000,
    queryFn: () => fetchYearEvents(calendarId!, year),
  });
}

/**
 * Year events from several calendars (the shared/extra year-grid sources).
 * One query per calendar, sharing cache keys with useYearEvents.
 */
export function useMultiYearEvents(calendarIds: string[], year: number) {
  const results = useQueries({
    queries: calendarIds.map((calId) => ({
      queryKey: yearEventsKey(calId, year),
      staleTime: 60 * 1000,
      queryFn: () => fetchYearEvents(calId, year),
    })),
  });

  const events = useMemo(
    () => results.flatMap((r) => r.data ?? []),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [...results.map((r) => r.data)],
  );

  return { events, isLoading: results.some((r) => r.isLoading) };
}
